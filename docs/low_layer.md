...
- プロセスの生成の目的は、同じプログラムの処理を複数のプロセスに分けて処理するか、別のプログラムを生成するかの2つある
- プロセスのライフサイクルは、fork(2)2で開始し、別のプログラムを生成する場合はexec(2)で別プロセスに変わり、wait(2)で親プロセスは子プロセスを待ち、exit(3)で終了する
- プロセスの状態は、生成後に実行可能状態となり、スリープ状態や実行状態間で遷移し、ゾンビ状態となり終了する
- コピーオンライトは、子プロセスは親プロセスとメモリアドレスを共有し、参照しているメモリが書き込まれる場合に、参照しているメモリをコピーして書き込まれる機構。fork()時は子プロセスにページテーブルのみがコピーされる
- プロセスの経過時間は、プロセスが開始してから終了するまでの経過時間のことで、timeコマンドのrealが示す
- プロセスの使用時間は、プロセスが実際に論理CPUを使用した時間のことで、timeコマンドのuser(ユーザーランドの時間)とsys(システムコール発行の時間)が示す
...
- 孤児プロセスとは、親プロセスが終了しても生きたままの子プロセスのこと
...
- 仮想記憶は、プロセスから物理アドレスに仮想アドレスを用いてアクセスする方法であり、メモリの断片化(フラグメンテーション)の防止ややメモリアクセスへの制御を担っている
- 仮想アドレス空間は、プロセスごとに割り当てられた仮想のメモリアドレスのこと
- メモリ管理ユニット(MMU)は、仮想メモリと物理メモリとマッピングするための機構で、ページテーブルという階層型のデータ構造によって実現している
- デマンドページングは、mmap()呼び出し後、新規獲得したメモリ領域にアクセスした際に物理メモリを割り当てる機能
...
- ヒュージページは、通常より大きい単位のページテーブルで、仮想メモリを多く使うプロセスに対して、ページテーブルを階層化することで使用するメモリ量を減らす機能
...
- ファームウェアはデバイス(コンピューターなど)を動かすために内装されている制御用ソフトウェアのこと
- BIOSは、プログラムの動作環境であり、コンピュータ起動時にブートストラップローダプログラムを起動し、OS(カーネル)をメモリに読み込み実行する
- コンピューターの電源が入ると、BIOSが起動しハードウェアを初期化し、ブートローダーがLinuxカーネルを起動し、カーネルがinitプロセスを起動し、initプロセスが子プロセスを起動する
