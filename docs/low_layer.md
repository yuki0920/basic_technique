...
## ファイルシステム
- ファイルシステムは、OSが提供するデータの格納場所、空き領域の場所を管理する機構。Linuxはファイルとして扱うことでストリームを利用できる
...
- デバイスファイルは、Linux上で動作しているハードウェアデバイスを表現したファイル
- キャラクタデバイスは、読み書きができるデバイスファイルで、端末(ターミナル)やキーボード、マウスがある
- ブロックデバイスは、読み書きとランダムがアクセスができるデバイスファイルで、HDDやSSDがある
...
- コンテナは、cgroup(プロセスごとのCPU,メモリのリソース割り当てを制御することができる機構)やnamespaceを利用して実現されている
